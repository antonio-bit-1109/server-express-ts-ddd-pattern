...
const UserModel_js_1 = __importDefault(require("../database/models/UserModel.js"));
async function Add_CampoPassword_to_User() {
    try {
        console.log("migrazione in corso...");
        console.log("aggiunta campo password allo user...");
        const result = await UserModel_js_1.default.updateMany({ Password: { $exists: false } }, { $set: { Password: "defaultPassword" } });
        if (result) {
